			...
			<tbody>
				...
				<? }else{ ?>
					...
					<?php if($value['Filed'][0] !='')
					{
						...
						<? echo "<span style='font-size:11px;color:blue;'><a href='/posts/search_restart_mail/".$value['Post']['id']."' target='_blank'/>еще</a></span> </span>";?>
						</div></td>
						...
						<td width="70">
							...
							<?=$this->Html->link($this->Html->image("curl.png", array("alt" => "Запустить")),array('action'=>'/view_order_one/'.$value['Post']['id'].''),array('escape' => false,'class' => 'icon','title'=>'Запустить'))?>		
					...
					<?php  if($value['Filed'][0] !=''){
						...
						echo " || <span style='font-size:12px;font-weight:700'>".$val['table'].'.'.$val['label'];
						if ($val['password']==''){
						echo "<span style='font-size:15px;color:blue;'> НЕ ИСКАЛИ/span>";
							
						}elseif($val['password']==':'){
							echo "<span style='font-size:15px;color:blue;'>  НЕ НАЙДЕНО  <a href='/posts/search_restart_pass/".$val['id']."' target='_blank'/>искать еще? >>></a></span> </span>";
						}else{
