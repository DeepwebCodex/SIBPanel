<!-- START CONTENT -->
	<div id="content">
		<ul class="page-nav fl">
			...
			<li class="active"><?=$html->link('txt (просто мыла) ',array('action'=>'download_domens2')); ?></li>
		</ul>
					...
					if($domain_name != $name[0] AND strpos($name[0],'txt'))
					{
						...
						if($size > 10)
						{
							...
							echo "<tr>
								...
								<td>{$date}</td>
								<td>{$result} мбайт</td>
								<td><a class='btn btn_green' href='./download_domens2/?name=$domain_name'>Скачать</a></td>
							</tr>";
						}
					}
				}
